...
readSnpsPed <- function(pedfile, mapfile) {
  ...
  if (any(is.na(snpobj))) {
    ...
    impute <- readline(
      paste(
        "This dataset contains NAs. Please select an imputation method",
...
resist_components <- function(
  ...
  if (!requireNamespace("vegan", quietly = TRUE)) {
    message(
      paste0(
        "The 'vegan' package is required for this function. Would you ",
    ...
    if (tolower(response) == "yes") {
      ...
      stop(
        paste0(
          "The 'vegan' package is needed for this function. Please ",
  ...
  if (!requireNamespace("ape", quietly = TRUE)) {
    message(
      paste0(
        "The 'ape' package is required for this function. Would you ",
        "like to install it? (yes/no)"
      )
    )
    response <- readline()
    if (tolower(response) == "yes") {
      ...
      stop(
        paste0(
          "The 'ape' package is needed for this function. Please ",
          "install it to proceed."
        )
      )
    }
  ...
  final_d <- lapply(
    ...
    function(i) {
      ...
      #plot it
      Tr_PcoA <- ggplot2::ggplot(
        # Would be good to make this 3D at some point
        PcoA2D_AT,
