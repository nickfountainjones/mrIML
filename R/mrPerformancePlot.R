...
mrPerformancePlot <- function(ModelPerf1 = NULL,
  ...
  if (mode == "classification") {
    ...
    model1_df <- m1 %>%
      tibble(model_type = rep(mod_names[1], d_length)) %>%
    ...
    model2_df <- m2 %>%
      tibble(model_type = rep(mod_names[2], d_length)) %>%
      dplyr::rename(metric = mcc) %>%
  ...
  if (mode == "classification") {
    p2 <- ggplot(
      long_df, 
      aes(
        x = reorder(response, difference, decreasing = T),
        y = difference
      )
    ) +
      ...
      labs(
        x = "Response",
        y = "Difference in MCC",
        title = paste0(mod_names[2], " vs ", paste(mod_names[1]))
      ) +
      theme_bw()
  } else {
    p2 <- ggplot(
