...
library(mrIML)
pacman::p_load(
  MRFcov, tidyverse, future.apply, tidymodels, finetune, themis, hstats,
  vip, flashlight, iml, vivid, igraph, ggnetwork, network, gridExtra, progress,
  xgboost, brulee, fastshap, tabnet, bonsai, parsnip, cowplot, lme4, geosphere
)
```

```{r include=FALSE}
if (Sys.getenv("NOT_CRAN") == "true") {
  plan(multisession)  # Use parallel only if not on CRAN
} else {
  plan(sequential)  # Force sequential processing on CRAN
}
```
...
Y <- MRFcov::Bird.parasites %>%
  ...
  dplyr::select(order(everything()))#response variables eg. SNPs, pathogens, species....
...
X1_fact <- X1 %>%
  ...
  dplyr::mutate_all(~ifelse(. == 0, "absent", "present"))
```
...
bs_malaria <- mrBootstrap(yhats_rf,
                          num_bootstrap = 100,
                          downsample = TRUE)#yhats=yhats_rf,
                          #Y=Y, 
                          #num_bootstrap = 10,
                          #downsample = TRUE,
                          #mode='classification') 
...
```{r, message = FALSE, warnings = FALSE}
covar <- mr_Covar(yhats=yhats_rf$Fits,#_downSamp,
                  X=X,
                  X1=X1,
                  Y=Y,
                  var='scale.prop.zos',
...
```{r, message = FALSE, warnings = FALSE}
int_ <- mrInteractions(yhats=yhats_rf$Fits,
                       X,
                       Y,
                       num_bootstrap=10,
                       feature = 'Plas',
